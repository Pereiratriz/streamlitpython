...
st.line_chart(data=leituras, x='Datetime', y='HumOut', color='#e5ff00')
st.write(f'Umidade mínima: {leituras.HumOut.min()} %')
st.write(f'Umidade máxima: {leituras.HumOut.max()} %')
st.write('---')
st.subheader('Radiação Solar')
st.line_chart(data=leituras, x='Datetime', y='SolarRad', color='#0000ff')
...
folium.Marker([-7.000126399270931, -42.10095318786533],
              popup='Estação Meterológica UAPP - IFPI',
              tooltip='Estação Meterológica UAPP - IFPI'
              ).add_to(m)
